...
var Canvas2dRenderer = (function Canvas2dRendererClosure() {
  ...
  Canvas2dRenderer.prototype = {
    ...
    _colorize: function () {
      ...
      // 现在代码
      // 不要覆盖或重定义只读的 ImageData.data。直接写入或使用 set 拷贝。
      try {
        if (typeof img.data.set === 'function') {
          img.data.set(imgData);
        }
      } catch (e) {
        // 兼容极端环境（通常不需要，因为 imgData 已经指向同一缓冲区）
        for (var i2 = 0; i2 < imgData.length; i2++) {
          img.data[i2] = imgData[i2];
        }
      }
